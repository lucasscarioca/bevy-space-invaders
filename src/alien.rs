...
// Marker component to prevent querying any dead aliens in our updates after they have already died
#[derive(Component)]
pub struct Dead;

// Controls the behaviour of our aliens
#[derive(Resource)]
...
fn update_aliens(
  ...
  for (entity, alien, mut transform, mut visibility) in alien_query.iter_mut() {
    ...
    if transform.translation.x.abs() > resolution.screen_dimensions.x * 0.5 {
      ...
      alien_manager.dist_from_boundary = resolution.screen_dimensions.x * alien_manager.direction * 0.5 - transform.translation.x; // calculate the delta x we need to move the alien to get it back into our bounds
    }
    ...
    // If the alien have made it out of the bottom of the screen whe have Lost the game and should reset
...
fn manage_alien_logic(
  ...
  if alien_manager.shift_aliens_down {
    ...
    for (_entity, mut _alien, mut transform) in alien_query.iter_mut() {
